...
public class ExcelAPI {
  ...
  public Cell getCell(Sheet sheet, int x, int y) {
    ...
    for (Row row : (XSSFSheet)sheet) {
      ...
      for (Cell cell : row) {
        if (cell.getAddress().getColumn() == x &&
            cell.getAddress().getRow() == y) {
          return cell;
        }
      }
      i++;
  ...
  public void writeCell(Cell cell, Object toWrite) {
    ...
    case "boolean":
      cell.setCellValue((boolean)toWrite);
      break;
    case "string":
      cell.setCellValue((String)toWrite);
      break;
    case "double":
      cell.setCellValue(Double.parseDouble((String)toWrite));
      break;
    case "integer":
      cell.setCellValue(Integer.parseInt((String)toWrite));
      break;
    case "date":
      cell.setCellValue(Date.parse((String)toWrite));
      break;
    }
  }
}
